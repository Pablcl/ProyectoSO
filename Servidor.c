...
typedef struct {
	...
	char estado[10];//Aceptar o rechazar
}invitados;
typedef struct {
	char invitador[100];
	invitados inv[3];
...
//Lista Jugador
int PonJugadores(listaPartidas *listaP, char nInvitador[100], char nInvitado[100])
{
	if(listaP-> numjugadores == 100)
		return -1;
	else{
		strcpy(listaP->jugadores[listaP->numjugadores].invitador,nInvitador);
		
		char *token;
		int i = 0;
		token = strtok(nInvitado, ",");
		while (token != NULL && i < 3)
		{
			strcpy(listaP->jugadores[listaP->numjugadores].inv[i].invitado, token);
			token = strtok(NULL, ",");
			i++;
		}
		listaP-> numjugadores++;
...
int PonEstados(listaPartidas *listaP, char nInvitador[100], char nInvitado[100], char nestado[5])
{
	if(listaP-> numjugadores == 100)
		return -1;
	else{
		...
		for (int j = 0; j < listaP->numjugadores && !encontrado; j++) 
		{
			if (strcmp(listaP->jugadores[j].invitador, nInvitador) == 0) 
			{
				for (int i = 0; i < 3; i++) 
				{
					if (strcmp(listaP->jugadores[j].inv[i].invitado, nInvitado) == 0) 
					{
						strcpy(listaP->jugadores[j].inv[i].estado, nestado);
						encontrado = true;
						break;
					}
				}
...
int DamePosicionJugadores (listaPartidas *listaP, char nInvitador[100])
{
	int i =0;
	int encontrado =0;
	while ((i<listaP-> numjugadores)&&!encontrado)
	{
		if (strcmp(listaP->jugadores[listaP->numjugadores].invitador, nInvitador)==0)
			encontrado =1;
		if (!encontrado)
			i=i+1;
	}
...
int EliminarJugadores(listaPartidas *listaP, char nombre[20])
{
	int pos = DamePosicionJugadores (listaP, nombre);
	if (pos == -1)
	{
		return -1;
	}
...
void DameInvitador (listaPartidas *listaP,char ninvitado, char invitador[300])
{
	bool encontrado = false;
	for (int j = 0; j < listaP->numjugadores && !encontrado; j++) 
	{
			...
			for (int i = 0; i < 3; i++) 
			{
				if (strcmp(listaP->jugadores[j].inv[i].invitado, ninvitado) == 0) 
				{
					strcpy(invitador, listaP->jugadores[j].invitador);
					encontrado = true;
					break;
				}
			}
...
void *AtenderCliente(void *sock)
{
	...
	while (terminar == 0) 
	{
		...
		{
			//formato: 6/nombre1,nombre2,nombre3
			char usuariosInvitados6[100];
			char *x1 = strtok(NULL, "/");
			strcpy (usuariosInvitados6, x1);// fromato: nombre1,nombre2,nombre3
			//separar la cadena de nombres
			char *nombreInvitado;
			int i = 0;
			nombreInvitado = strtok(usuariosInvitados6, ",");
			while (nombreInvitado != NULL && i < 3)
			{
				PonJugadores(&milistaP, userID, nombreInvitado);
				if(usuariosInvitados6 == NULL)//no se ha invitado a ningun usuario
				{
				...
				{
					sprintf (respuesta,"6/%s,%s",userID,nombreInvitado);//6/invitador,invitado
		...
		{
			//formato: 65/yes or no,nombre invitado
			char resinvitado[100]; //respuesta del invitado
			char *x2 = strtok(NULL, ","); 
			strcpy (resinvitado, x2);
			char nombreInvitado65[100];
			char *x3= strtok(NULL, ",");
			strcpy(nombreInvitado65, x3);
			int res;
			char invitador[200];
			printf("resinv: %s\n", resinvitado);
			if(strcmp(resinvitado, "yes")==0)
			{
				res = PonEstados(&milistaP, userID, nombreInvitado65, resinvitado);
				if (res ==0)
				{
					DameInvitador(&milistaP, nombreInvitado65, invitador);
					printf ("Jugador %s ha aceptado tu invitacion, %s, resinv: %s\n", nombreInvitado65, invitador, resinvitado);
					sprintf (respuesta,"65/yes,%s,%s\n",nombreInvitado65, invitador);//65/yes,usuarioInvitado,usuario invitador
				}
			}
			else
			{
				res = PonEstados(&milistaP, userID, nombreInvitado65, resinvitado);
				printf ( "Jugador %s ha rechazado tu invitacion, resinv: %s\n", nombreInvitado65, resinvitado);
				sprintf (respuesta,"65/no,%s,%s\n", nombreInvitado65, invitador);
			}
			   int j;
...
/*				for(int j = 0; j < milista.num; j++)*/
/*				{*/
/*					if(strcmp(milista.conectados[j].nombre, usuarioInvitado6) == 0) */// encontrar el socket del oponente					
/*					{*/
/*						receiver_socket = milista.conectados[j].socket;*/
/*						break;*/
/*					}*/
...
/*				printf("sender:%s, receiver:%s",sender_socket, receiver_socket);*/
				// Formato: "66/sender/message"
/*				sprintf(respuesta, "66/%s/%s", userID, message);*/
