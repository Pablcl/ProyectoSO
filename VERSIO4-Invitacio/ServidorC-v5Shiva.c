	...
	while (terminar == 0) 
	{
		...
		else if (codigo == 1) //INICIAR SESION
		{
			...
			if (row == NULL) {
				...
				sprintf(respuesta,"1/Se ha iniciado sesion correctamente, bienvenido %s\n", row[0]);
...
/*			row = mysql_fetch_row(resultado);*/
		...
		//QUERY: ENCONTRAR LOS JUGADORES QUE EN UN ID PARTIDA DADA
			...
			if (row == NULL) {
				...
				sprintf(respuesta, "3/Nombre de las personas con las que has jugado: %s, %s", row[0], row[1]);
			}
		...
		////QUERY: LAS PARTIDAS QUE HE JUGADO EN EL DIA INTRODUCIDA
			...
			if (mysql_num_rows(resultado) == 0) {
				...
				while ((row = mysql_fetch_row(resultado)) != NULL) {
					char temp[256];
					sprintf(temp, "4/GAMEID: %s, GAME_DATE: %s, GAME_TIME: %s, DURATION: %s, WINNER: %s\n",
		...
		else if (codigo == 6)
		{
			...
			while (invitado != NULL)
			{
				sprintf(respuesta, "6/%d/%s/%s",idPartida, anfitrion6, invitado);
		...
		{
			//formato: 65/yes or no/anfitrion/nombre invitado /idpartida
		...
		else if (codigo == 10)
		{
			...
			//respuesta que envia a los clientes : 10/anfitrion/tirador/todos los invitados,
			
			char Cpynameresult10[1000];
			char resultadoTodosJugadores[400] = "";
			strcpy(Cpynameresult10, nameresult10);
			char *x6 = strtok (Cpynameresult10, "/");
			while (x6!=NULL)
			{
				sprintf(resultadoTodosJugadores, "%s%s,", resultadoTodosJugadores, x6);
				x6 = strtok(NULL, "/");
			}
			
			sprintf(respuesta, "10/%s/%s/%s/%d/%d/%d", anfitrion10, tirador10, resultadoTodosJugadores, numPregunta10, resultado10, idpartida10);	
